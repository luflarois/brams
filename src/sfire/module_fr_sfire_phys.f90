...
PUBLIC:: fire_wind_height,fcz0,fcwh,have_fuel_cats,nfuelcats,no_fuel_cat,no_fuel_cat2,windrf,moisture_classes,itree,fueldepthm
PUBLIC:: mfuelcats
...
do j=jts,jte
 ...
 !       print*,'ADVANCE fmep',  fmep(i,k,j)
 !       call flush(6)
      else
        fmep(i,k,j) = fmep(i,k,j)*exp(-change)
  !      print*,'ADVANCE fmep 2',  fmep(i,k,j)
  !      call flush(6)
      endif
    enddo
  enddo
...
real, dimension(1:3,1:nsteps), target::vx,vy,zsf,dzdxf,dzdyf,bbb,phisc,phiwc,r_0,fgip,ischap,fmc_g,wind,nfuel_cat,&
                                       bbb_FM10,phisc_FM10,phiwc_FM10,r_0_FM10,CFB,HPA ! INTRODUZIDO POR ISILDA CM
...
!!!!!! INTRODUZIDO POR ISILDA CUNHA MENEZES
   ...
   !  print*,"estou no modulo_fr_drive_phys e estou na rotina set_fire_crown_params"
   !  call flush(6)
...
!$OMP CRITICAL(SFIRE_PHYS_CRIT)
          write(msg,'(a,2i6,3(a,e13.5))') 'fire_ros_cawfe: at ',i,j,' rate of spread',ros_FM10,' moisture ',fp%fmc_g(i,j),'> extinction =',fuelmce(k)
!$OMP END CRITICAL(SFIRE_PHYS_CRIT)
