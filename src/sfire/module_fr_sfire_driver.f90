    ...
    if(fire_run)then
     ...
     if(ifun.eq.2)then   
      ...
      if(restart)then
        ...
        if(kfmc_ndwi > 0 .and. fndwi_from_ndwi .eq.1)then
            print*, "estou dentro da rotina fire_driver_em dentro da rotina driver_phys no ifun=2 e vou chamar a rotina print_2d_stats"
            call flush(6)
            ...
            call interpolate_z2fire(id,0,                 & 
                ...
                ndwi,                                       & 
                fndwi)
            print*,"estou dentro da rotina fire_driver_em na rotina driver_phys no ifun=2 e vou chamar a rotina print_2d_stats 2"
            call flush(6)                
...
!!$OMP END CRITICAL(SFIRE_DRIVER_CRIT)
         ...
         print*,"tou dentro da rotina fire_driver_em no ifun=2 e vou chamar a rotina sfire_driver_phys e vou entrar na rotina print_array_m"
         call flush(6)
        ...
        if (interpolate_long_lat)then
         ...
         print*, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys e vou entrar na rotina interpolate z2fire 2"
         call flush(6)
        ...
        print*, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys e vou entrar na rotina print_2d_stats 1"
        call flush(6)
...
!!!!!!!!
       ...
       !   call flush(6)
          ...
          print*,"estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina interpolate_atm2fire"
          call flush(6)
        ...
        !    print*,'TVYYY',fp%vy
        !    call flush(6)                                    
          print*, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina apply_windrf"
          call flush(6)
          call apply_windrf(                        &
            ifms,ifme,jfms,jfme,                    &
            ifts,ifte,jfts,jfte,                    &
        ...
        !  call flush(6)
          ...
          print*, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina interpolate_wind2fire_height"
          call flush(6)        
         ...
         !   call flush(6)                            
          ...
          if(fire_use_windrf.eq.1)then
            print*, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina apply_windrf 2"
            call flush(6)
            call apply_windrf(                      &
            ifms,ifme,jfms,jfme,                    &
            ifts,ifte,jfts,jfte,                    &
          ...
          if(fire_use_windrf.eq.4)then
            ...
            if(fire_can_top_read == 0)then
                ...
                can_top = 0                             
                print*,"estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina find_trees_fmesh"
                call flush(6)
            ...
            print*, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina interpolate_wind2fire_height 3"
            call flush(6)
            ...
            print*,"estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina massman_fwh"
            call flush(6)
            call massman_fwh(id,                          & 
            ids,ide, kds,kde, jds,jde,                    & 
            ims,ime, kms,kme, jms,jme,                    &
    ...
    elseif(ifun.eq.4)then
      ...
      if(run_fuel_moisture)then
        ...
        print*, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e vou entrar na rotina fuel_moisture"
        call flush(6)
        call fuel_moisture(                &
        id,                                     & 
        nfmc,                                &
    ...
    call sfire_model (id,ifun,restart,replay,  &
     ...
     if(ifun.eq.2)then
        print*, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys e vou entrar na rotina setup_wind_log_interpolation"
        call flush(6)
    ...
    print*, "estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys e vou entrar fire_rate_of_spread para calcular o f_ros"
    call flush(6)
    do j=jfts,jfte
       do i=ifts,ifte
       ...
       enddo
    enddo
    print*,"estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys e vou entrar no print_2d_stats f_ros"
    call flush(6)
...
!$OMP end CRITICAL(SFIRE_DRIVER_CRIT)
	  call message(msg)
          s = 1./(ir*jr)
          print*,"estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys e vou calcular os avg_fuel_frac grnhfx grnqfx"
          call flush(6)
          do j=jts,jte
            do i=its,ite
