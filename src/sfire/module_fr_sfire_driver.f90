module module_fr_sfire_driver
   ...
   use module_fr_sfire_atm, only: read_emissions_table, add_fire_emissions !!!!!!!!!!!! ATENCAO AQUI E PARA TIRAR
...
91       format('Time ', f11.3, ' s ', a, e12.3, 1x, a)
      ...
      call print_2d_stats((ifds + ir), (ifde - ir), (jfds + jr),(jfde - jr),ifms, ifme, jfms, jfme, grid%tign_g &
                          , 'sfire_driver_em: grid%tign_g')
      call print_2d_stats((ifds + ir),(ifde - ir), (jfds + jr),(jfde - jr), ifms, ifme, jfms, jfme,grid%nfuel_cat &
...
95    format('namelist.input restart ', l1, ' cycling ', l1, ' fire_restart ', l1, ' setting restart ', l1)
      ...
      do istep = 0, tsteps
         ...
         do fire_ifun = fire_ifun_start, fire_ifun_end
            ...
            call sfire_driver_phys( &
               ...
               grid%uah, grid%vah, &
               grid%fgrnhfx, grid%fgrnqfx,& !grid%fcanhfx, grid%fcanqfx, & COMENTADO POR ISILDA CM
            ...
            if (fire_ifun .eq. 6) then
               ...
               if (config_flags%chem_opt > 0 .or. config_flags%tracer_opt > 0) then
                  ...
                  call add_fire_emissions(config_flags%tracer_opt, dt,&
                                          grid%dx, grid%dy, &
                                          ifms, ifme, jfms, jfme, &
                                          ifds, ifde, jfds, jfde, &
                                          ids, ide, kds, kde, jds, jde, &
                                          ims, ime, kms, kme, jms, jme, &
                                          rho, dz8w, &
                                          grid%fgip, grid%fuel_frac_burnt,&
                                          grid%nfuel_cat,grid%chem, grid%tracer)
   ...
   subroutine sfire_driver_phys(ifun, &
                                ...
                                uah, vah, &
                                fgrnhfx, fgrnqfx,& !fcanhfx, fcanqfx, & COMENTADO POR ISILDA CM
      ...
      real, intent(out), dimension(ifms:ifme, jfms:jfme):: &
         fgrnhfx, &
         fgrnqfx, &
        ! fcanhfx, & COMENTADO POR ISILDA CM
        ! fcanqfx, & COMENTADO POR ISILDA CM
         ros, flineint, flineint2, &
         f_ros0, f_rosx, f_rosy, f_ros, &
         f_int, f_lineint, f_lineint2, &
         f_ros11, f_ros12, f_ros13, f_ros21, &
      ...
      integer :: ij, i, j, k, id, pid, ipe1, jpe1, ite1, jte1, &
                 ii, jj
      ...
      if (ifun .eq. 2) then
         print*, "print_chsum u"
         call flush (6)
         call print_chsum(itimestep,ims,ime,kms,kme,jms,jme,ids,ide,kds,kde,jds,jde,u,'u')
          print*, "print_chsum v"
         call flush (6)
         call print_chsum(itimestep,ims,ime,kms,kme,jms,jme,ids,ide,kds,kde,jds,jde,v,'v')
          print*, "print_chsum ph"
         call flush (6)
         call print_chsum(itimestep,ims,ime,kms,kme,jms,jme,ids,ide,kds,kde,jds,jde,ph,'ph')
      end if
       print*, "print_chsum lfn"
         call flush (6)
      call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,lfn,'lfn')
       print*, "print_chsum tign"
         call flush (6)
      call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,tign,'tign')
       ...
       !  print *, 'LFR-DBG: run_advance_moisture=',run_advance_moisture
         ...
         if (run_advance_moisture) then
            ...
            if (ifun .eq. 3) then
               ...
               print *, "estou dentro da rotina fire_driver_em no ifun=3 e vou chamar a &
                          rotina advance_moisture calcular fmc_equi fmc_lag"
               call flush (6)
               
               call advance_moisture(itimestep .eq. 1, &
                                     ims, ime, jms, jme, &
                                     ids, ide, jds, jde,&
                                     nfmc, dt_moisture, &
                                     fmep_decay_tlag, &
               ...
               print*, "sai da advance_moisture calculei fmc_equi fmc_lag"
               call flush (6)                      
         ...
         print *, "estou dentro da rotina fire_driver_em no ifun=3 e estou dentro da rotina &
                           sfire_driver_phys estou fora advance_moisture"
         call flush (6)
               ...
               if (restart) then
                  ...
                  if (kfmc_ndwi > 0 .and. fndwi_from_ndwi .eq. 1) then
                     ...
                     print *, "estou dentro da rotina fire_driver_em dentro da rotina driver_phys no ifun=2 e vou &
                         chamar a rotina print_2d_stats fndwi", size(fndwi,1),size(fndwi,2),ifms,ifme,jfms,jfme
                     call flush (6)
                     
                     call print_2d_stats(ids, ide, jds, jde, ims, ime, jms, jme, fndwi, 'driver:ndwi')
                     
                     print *, "estou dentro da rotina fire_driver_em no ifun=2 e vou chamar &
                                   a rotina interpolate z2fire calcular ndwi fndwi"
                     call flush (6)
                     ...
                     call interpolate_z2fire(id, 0, &
                                             ...
                                             ndwi, &
                                             fndwi)
                     print*, "sai interpolate_z2fire calculei ndwi fndwi"
                     print *, "estou dentro da rotina fire_driver_em na rotina driver_phys&
                             no ifun=2 e vou chamar a rotina print_2d_stats 2"
                     call flush (6)
                     call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fndwi, 'driver:fndwi')
                  end if


                  print *, "tou dentro da rotina fire_driver_em no ifun=2 e vou chamar a &
                         rotina sfire_driver_phys e vou entrar na rotina print_array_m"
                  call flush (6)
                   print*, "write xlat"
                   call flush (6)
                  call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme,xlat, 'xlat', id)
                  print*, "write xlong"
                   call flush (6)
                  call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme, xlong, 'xlong', id)
                  
                  if (interpolate_long_lat) then
                  
                     call message('Interpolating node longitude and latitude to fire mesh')
                     print*, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys&
                              e vou entrar na rotina interpolate z2fire 2 fxlat"
                     call flush (6)
                     ...
                     print*, "vou entrar na rotina interpolate z2fire 3 e levo FXLAT"
                     call flush (6)
                    ! do i=1,size(fxlat,1)
                    !   do j=1,size(fxlat,2)
                    !      write(41,*) i,j, fxlat(i, j)
                    !    end do
                    ! end do
                    ! print*,"FXLAT   ", fxlat 
                    ! call flush (6)
                     ...
                     print*, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys &
                           e vou entrar na rotina interpolate z2fire 2 fxlong"
                     ...
                     call interpolate_z2fire(id, 1, &
                                             ...
                                             fxlong)
                  print *, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys &
                        e vou entrar na rotina print_2d_stats 1 com ZSF  e levo FXLONG"
                  call flush (6)
                 ! do i=1,size(fxlong,1)
                 !      do j=1,size(fxlong,2)
                 !         write(42,*) i,j, fxlong(i, j)
                 !       end do
                 !  end do  
                  !print*, "FXLONG    ", fxlong
                  !call flush (6)
                 ...
                 !##### INTRODUZIDO POR ISILDA CM
                  print *,'build_ZSF'
                  call build_ZSF(ifms, ifme, jfms ,jfme,&
                              ifds, ifde, jfds ,jfde,&
                              fxlat, fxlong,fp%zsf)
                  
                  print *,'build_NFFL'            
                  call build_NFFL(ifms, ifme, jfms ,jfme,&
                              ifds, ifde, jfds ,jfde,&
                              fxlat, fxlong,nfuel_cat)

            do i = ifds, ifde
               do j =jfds, jfde
                  fp%dzdxf(i, j) = (fp%zsf(i + 1, j) - fp%zsf(i - 1, j))/(2.*dxf)
                  fp%dzdyf(i, j) = (fp%zsf(i, j + 1) - fp%zsf(i,j - 1))/(2.*dyf)
               end do
            end do                  
                 !######################
                 endif
 
                  print*, "print_2d_stats zsf"
                  call flush (6)
                 ! print*, "ZSF    ",fp%zsf
                 !  call flush (6)
                  call print_2d_stats(ifds, ifde, jfds, jfde,ifms, ifme, jfms, jfme, fp%zsf, 'driver_phys:zsf')
                  print*, "sai print_2d_stats zsf"
                  call flush (6)
               end if
...
!!!!!!!!!
                    ...
                    print*, "vou entrar na rotina write_array_m3 2 z0"
                    call flush(6)
                  call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme, z0, 'z0', id)
                    print*, "write_array_m3 2 - a u_2"
                    call flush(6)
                  call write_array_m3(ids, ide, kds, kde, jds, jde, ims, ime, kms, kme, jms, jme, u, 'u_2', id)
                    print*, "write_array_m3 2 - b v_2"
                    call flush(6)
                  call write_array_m3(ids, ide, kds, kde, jds, jde, ims, ime, kms, kme, jms, jme, v, 'v_2', id)
                    print*, "write_array_m3 2 - c ph_2"
                    call flush(6)
                  call write_array_m3(ids, ide, kds, kde, jds, jde, ims, ime, kms, kme, jms, jme, ph, 'ph_2', id)
                    print*, "write_array_m3 2 - d phb"
                    call flush(6)
                  call write_array_m3(ids, ide, kds, kde, jds, jde, ims, ime, kms, kme, jms, jme, phb, 'phb', id)
...
!!!!!!!!
                  ...
                  if (fire_wind_log_interp .eq. 4) then
                        ...
                        print*, "vou entrar na rotina print_2d_stats_4 z0"
                        call flush(6)
                     
                     call print_2d_stats(ids, ide, jds, jde, ims, ime, jms, jme,z0, 'driver_phys:z0')
                     
                     print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys &
                                  e vou entrar na rotina interpolate_atm2fire vx vy"
                     call flush (6)
                     ...
                     print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys &
                                e vou entrar na rotina apply_windrf vx vy"
                     call flush (6)
                     
                     call apply_windrf( ifms, ifme, jfms, jfme, &
                                        ifds, ifde, jfds, jfde, &
                     ...
                     wz0(:, :) = 0.0
                       ...
                       print*,"estou ca fora da rotina windrf e wz0 e "

                       call flush(6)
                     
                  else
                     print*, "print_2d_stats B fz0"
                     call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fz0, 'driver_phys:fz0')

                     
                     print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys e &
                           vou entrar na rotina interpolate_wind2fire_height vx vy"
                     call flush (6)
                     ...
                     if (fire_use_windrf .eq. 1) then
                        ...
                        print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys &
                               e vou entrar na rotina apply_windrf 2 vx vy"
                        call flush (6)
                     ...
                     if (fire_use_windrf .eq. 4) then
                        ...
                        print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys &
                          e vou entrar na rotina interpolate_wind2fire_height 3 cuf cvf"
                        call flush (6)
                        ...
                        print *, "estou dentro da rotina fire_driver_em no ifun=3 na rotina sfire_driver_phys&
                          e vou entrar na rotina massman_fwh vx vy"
                        call flush (6)
            ...
            elseif (ifun .eq. 4) then
               ...
               if (run_fuel_moisture) then
                  ...
                  print *, "estou dentro da rotina fire_driver_em no ifun=4 na rotina sfire_driver_phys &
                          e vou entrar na rotina fuel_moisture fmc_gc fmc_g"
                  call flush (6)
                  do k=1,size(fmc_gc,2)
                     do i=1,size(fmc_gc,1)
                       do j=1,size(fmc_gc,3)
                          write(43,*) k,i,j, fmc_gc(i,k,j)
                        end do
                     end do
                   end do
                  
                  call fuel_moisture( id, nfmc, &
                                      ids, ide, jds, jde, &
                                      ims, ime, jms, jme, &
                                      ifds, ifde, jfds, jfde, &
                                      ifms, ifme, jfms, jfme, &
               ...
               print*, "sai da fuel_moisture fmc_gc fmc_g"
               call flush (6)
                  
               end if


               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fmc_g, 'driver_phys:fmc_g')
            end if
            ...
            print *, " sai sfire_model"
            call flush (6)

            if (ifun .eq. 2) then
            
               print *, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys &
                  e vou entrar na rotina setup_wind_log_interpolation fz0 fwh wz0"
               call flush (6)
               
               call setup_wind_log_interpolation( ids, ide, jds, jde, &
                                                  ims, ime, jms, jme, &
                                                  ifds, ifde, jfds, jfde, &
                                                  ifms, ifme, jfms, jfme, &
                                                  ir, jr, &
            ...
            print*, "sai setup_wind_log_interpolation fz0, fwh, wz0"
            call flush (6)
            elseif (ifun .eq. 6) then

               print *, "estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys &
                    e vou entrar fire_rate_of_spread para calcular o f_ros"
               call flush (6)
               ...
               print *, "estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys&
                    e vou entrar no print_2d_stats f_ros"
               call flush (6)
               
            
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros11, 'driver_phys:f_ros11')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros12, 'driver_phys:f_ros12')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros13, 'driver_phys:f_ros13')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros21, 'driver_phys:f_ros21')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros23, 'driver_phys:f_ros23')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros31, 'driver_phys:f_ros31')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros32, 'driver_phys:f_ros32')
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, f_ros33, 'driver_phys:f_ros33')

              
                print*,"estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys &
                          e vou entrar no print_2d_stats ros"
                call flush(6)
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, ros, 'sfire_driver:ros')
               !print*,'IM-DBG: ROS =',ros
                 print*,"estou dentro da rotina fire_driver_em no ifun=6 na rotina sfire_driver_phys &
                        e vou entrar no print_2d_stats fgrnhfx"
                 call flush(6)
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fgrnhfx, 'sfire_driver:fgrnhfx')
              ! print*,'IM-DBG:DRIVER: fgrnhfx =',fgrnhfx
                 print*,"estou dentro da rotina fire_driver_em no ifun=6 na rotina &
                              sfire_driver_phys e vou entrar no print_2d_stats fgrnqfx"
                 call flush(6)
               call print_2d_stats(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fgrnqfx, 'sfire_driver:fgrnqfx')
              ! print*,'IM-DBG:DRIVER: fgrnqfx =',fgrnqfx

               print *, "estou dentro da rotina fire_driver_em no ifun=6 na &
                            rotina sfire_driver_phys e vou calcular os sum_2d_cells"
               call flush (6)
               
               if (use_atm_vars) then

                   print*,'sum_2d_cells avg_fuel_frac'
                    call flush(6)
                  ...
                  call message(msg)
                  !print*,"DRIVE -iFUN = ? - soma calores",ifun
                  
                  s = 1./(ir*jr)
                  
                  print *, "estou dentro da rotina fire_driver_em no ifun=6 na &
                 rotina sfire_driver_phys e vou calcular os avg_fuel_frac grnhfx grnqfx"
                  call flush (6)
                  ...
                  do j = jds, jde
                     do i = ids, ide
                      ...
                      !  if (grnhfx(i, j) .gt. 0.0)then
                      !  print*,'IM-DBG:DENTRO:i,j,grnqfx(i, j)= ',i,j,grnqfx(i, j)
                      !  print*,'IM-DBG:DENTRO:i,j,grnhfx(i, j)= ',i,j,grnhfx(i, j)
                      !  endif
      ...
      if (ifun .eq. 1) then
         ...
         if (pid .ne. 0) then
            print*, 'ifun= 1 write_array_m zs N'
            call flush (6)
            call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme, zs, 'zs', pid)
            print*, 'ifun= 1 write_array_m zsf N'
            call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fp%zsf, 'zsf', pid)
         end if
      end if
      ...
      if (ifun .eq. 2) then
         ...
         print *, "estou dentro da rotina fire_driver_em no ifun=2 na rotina sfire_driver_phys &
                    e vou chamar fire_ignition_convert"
         call flush (6)

         call message('Processing ignition lines')
        
         print*, "ifun=2 fire_ignition_convert fxlong fxlat"
         
         call fire_ignition_convert(config_flags, ignition, &
                                    fxlong, fxlat, &
                                    ifds, ifde, jfds, jfde, &
                                    ifms, ifme, jfms, jfme)
          print *, "ifun=2 sai fire_ignition_convert"
         call flush (6)
         print *, "ifun=2 sai fire_hfx_convert calcular hfx"
         call flush (6)                           
         call fire_hfx_convert(config_flags, hfx)
         print *, "ifun=2 sai fire_hfx_convert calculei hfx"
         call flush (6)
      ...
      if (ifun .eq. 3) then
         ...
         print *, "ifun=3 vou chamar a rotina stats_by_slice A9 fmc_gc"
         call flush (6)
         
 
       
         call print_3d_stats_by_slice(ids, ide, 1, moisture_classes, jds, jde, ims, ime, 1, nfmc, jms, jme, fmc_gc, 'fmc_gc')
         
! print*, "ifun=3 sai da rotina stats_by_slice A9"
! call flush(6)
         print *, "ifun=3 vou chamar a rotina print_chsum fmc_gc"
         call flush (6)
         
    !        VERIFICAR MELHOR O SENTIDO DESTA CALL QUE EU ALTEREI E QUE TINHA nmfl     
         call print_chsum(itimestep,ims,ime,1,nfmc,jms,jme,ids,ide,1,moisture_classes,jds,jde, fmc_gc, 'fmc_gc')
      end if

      if (ifun .eq. 4) then
         print *, "ifun=4 vou chamar a rotina print_chsum fmc_g"
         call flush (6)

         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,fmc_g,'fmc_g')
         print *, "ifun=3 vou chamar a rotina print_chsum uf"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,fp%vx,'uf')
         print *, "ifun=3 vou chamar a rotina print_chsum vf"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,fp%vy,'vf')
         print *, "ifun=3 vou chamar a rotina print_chsum lfn"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,lfn,'lfn')
         print *, "ifun=3 vou chamar a rotina print_chsum tign"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,tign,'tign')
        ...
        !!!!!!!! VAMOS GRAVAR DE HORA A HORA INTRODUZIDI POR ISILDA
            ...
            ! ka=ka+1.
        !!!!!!!!!
!###################ESTAO AQUI OS ipe1 e jpe1
            print *, "ifun=4 vou chamar a rotina write_array_m grnhfx"
         call flush (6)
            call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme, grnhfx, 'grnhfx', pid)
            print *, "ifun=4 vou chamar a rotina write_array_m grnqfx"
         call flush (6)
            call write_array_m(ids, ide, jds, jde, ims, ime, jms, jme, grnqfx, 'grnqfx', pid)
             print *, "ifun=4 vou chamar a rotina write_array_m u"
         call flush (6)
            call write_array_m3(ids, ipe1, kds, kde + 1, jds, jde, ims, ime, kms, kme, jms, jme, u, 'u', pid)
             print *, "ifun=4 vou chamar a rotina write_array_m v"
         call flush (6)
            call write_array_m3(ids, ide, kds, kde + 1, jds, jpe1, ims, ime, kms, kme, jms, jme, v, 'v', pid)
            print *, "ifun=4 vou chamar a rotina write_array_m uf"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fp%vx, 'uf', pid)
             print *, "ifun=4 vou chamar a rotina write_array_m vf"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fp%vy, 'vf', pid)
...
!!!!!!!!!!!!!!
      ...
      if (ifun .eq. 5) then
         print *, "ifun=5 vou chamar a rotina print_chsum lfn"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,lfn,'lfn')
         print *, "ifun=5 vou chamar a rotina print_chsum tign"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,tign,'tign')
         if (pid .gt. 0) then
        !!!!!!!! VAMOS GRAVAR DE HORA A HORA INTRODUZIDI POR ISILDA
            ...
            ! ka=ka+1.
        !!!!!!!!!
            print *, "ifun=5 vou chamar a rotina write_array_m lfn"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde,ifms, ifme, jfms, jfme, lfn, 'lfn', pid)
            print *, "ifun=5 vou chamar a rotina write_array_m tign"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde,ifms, ifme, jfms, jfme, tign, 'tign', pid)
      ...
      if (ifun .eq. 6) then
         ...
         !call flush (6)
         print *, "ifun=6 vou chamar a rotina print_chsum fgrnhfx"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,fgrnhfx,'fgrnhfx')
         print *, "ifun=6 vou chamar a rotina print_chsum fgrnqfx"
         call flush (6)
         call print_chsum(itimestep,ifms,ifme,1,1,jfms,jfme,ifds,ifde,1,1,jfds,jfde,fgrnqfx,'fgrnqfx')
         print *, "ifun=6 vou chamar a rotina print_chsum grnhfx"
         call flush (6)
         call print_chsum(itimestep,ims, ime, 1, 1, jms, jme, ids, ide, 1, 1, jds, jde, grnhfx, 'grnhfx')
         print *, "ifun=6 vou chamar a rotina print_chsum grnqfx"
         call flush (6)
         call print_chsum(itimestep,ims, ime, 1, 1, jms, jme, ids, ide, 1, 1, jds, jde, grnqfx, 'grnqfx')
         
          !### PEREIRA
          do i=1,size(fp%zsf,1)
             do j=1,size(fp%zsf,2)
                write(49,*) i,j, fp%zsf(i,j),fp%dzdxf(i,j),fp%dzdyf(i,j),nfuel_cat(i,j)
             end do
          end do
         !##########
         ...
         if (pid .gt. 0) then
            ...
            print *, "estou dentro da rotina fire_driver_em no ifun=6 na rotina &
                          sfire_driver_phys e vou escrever od arrays_m 5"
            call flush (6)
        ...
        !!!!!!!! VAMOS GRAVAR DE HORA A HORA INTRODUZIDI POR ISILDA
            ...
            ! ka=ka+1.
        !!!!!!!!!
            print *, "ifun=6 vou chamar a rotina write_array_m grnhfx"
         call flush (6)      
            call write_array_m(ids, ide, jds, jde,ims, ime, jms, jme, grnhfx, 'grnhfx', pid)
            print *, "ifun=6 vou chamar a rotina write_array_m grnqfx"
         call flush (6)
            call write_array_m(ids, ide, jds, jde,ims, ime, jms, jme, grnqfx, 'grnqfx', pid)
             print *, "ifun=6 vou chamar a rotina write_array_m fuel_frac"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme,fuel_frac, 'fuel_frac', pid)
             print *, "ifun=6 vou chamar a rotina write_array_m fgrnhfx"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme,fgrnhfx, 'fgrnhfx', pid)
            print *, "ifun=6 vou chamar a rotina write_array_m grnqfx"
         call flush (6)
            call write_array_m(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme,fgrnqfx, 'fgrnqfx', pid)
  ...
  subroutine check_fmesh(ids, ide, ifds, ifde, ir, s)
   ...
   ! Verifica se a rela��o entre os �ndices da malha principal e os sub�ndices do fire mesh
   ! � consistente com o fator de refinamento (ir).
...
1     format('module_fr_sfire_driver: incompatible bounds ', a, ' atm ', i5, ':', i5, ' fire ', i5, ':', i5, ' ratio ', i3)
   ...
   subroutine fire_ignition_convert(config_flags, ignition, &
    ...
    !  print*,'IM-DBG SIM REAL:Using real ignition coordinates, longitude and latitude'
      ...
      corner_longlat(1, 1, 1) = fxlong(ifds, jfds)
      print*,'corner_longlat(1, 1, 1) =',  corner_longlat(1, 1, 1)
      corner_longlat(1, 1, 2) = fxlat(ifds, jfds)
      print*,'corner_longlat(1, 1, 2) =',  corner_longlat(1, 1, 2)
      corner_longlat(1, 2, 1) = fxlong(ifds, jfde)
      print*,'corner_longlat(1, 2, 1) =',  corner_longlat(1, 2, 1)
      corner_longlat(1, 2, 2) = fxlat(ifds, jfde)
      print*,'corner_longlat(1, 2, 2) =',  corner_longlat(1, 2, 2)
      corner_longlat(2, 1, 1) = fxlong(ifde, jfds)
      print*,'corner_longlat(2, 1, 1) =',  corner_longlat(2, 1, 1)
      corner_longlat(2, 1, 2) = fxlat(ifde, jfds)
      print*,'corner_longlat(2, 1, 2) =',  corner_longlat(2, 1, 2)
      corner_longlat(2, 2, 1) = fxlong(ifde, jfde)
      print*,'corner_longlat(2, 2, 1) =',  corner_longlat(2, 2, 1)
      corner_longlat(2, 2, 2) = fxlat(ifde, jfde)
      print*,'corner_longlat(2, 2, 2) =',  corner_longlat(2, 2, 2)
   ...
   contains
      ...
      subroutine display_clamp
         character(len=128)::msg
         real::d1, d2
         if (ii >= ifds .and. ii <= ifde .and. jj >= jfds .and. jj <= jfde) then
...
!subroutine set_fp_from_grid(grid,fp)
   ...
   subroutine check_grid_alloc(grid, config_flags)
      ...
      !LFR BUG call check_size(size(grid%chem), 'chem', config_flags%chem_opt)
      !LFR BUG call check_size(size(grid%tracer), 'tracer', config_flags%tracer_opt)
